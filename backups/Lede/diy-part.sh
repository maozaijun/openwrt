...
# 后台IP设置
export Ipv4_ipaddr="192.168.8.2"            # 修改openwrt后台地址(填0为关闭)
...
# 旁路由选项
export Gateway_Settings="192.168.8.2"        # 旁路由设置 IPv4 网关(填入您的网关IP为启用)(填0为不作修改)
export DNS_Settings="255.255.255.0"             # 旁路由设置 DNS(填入DNS，多个DNS要用空格分开)(填0为不作修改)
...
export Customized_Information="猫仔菌 $(TZ=UTC-8 date "+%Y.%m.%d")"  # 个性签名,你想写啥就写啥，(填0为不作修改)
